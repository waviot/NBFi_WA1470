...
static void addmod512(uint8_t *dst, uint8_t *src, uint8_t *add)
{
	uint8_t i;
	uint16_t overrun = 0;

	for (i = STRIBOG_BLOCK_SIZE; i-- > 0;) {
		overrun = src[i] + add[i] + (overrun >> 8);
		dst[i] = (uint8_t) overrun;
	}
}

static void addmod512_uint32_t(uint8_t *dst, uint8_t *src, uint32_t add)
{
	uint8_t i;
...
static void xor512(uint8_t *dst, const uint8_t *a, const uint8_t *b)
{
	uint8_t i;

	for (i = 0; i < STRIBOG_BLOCK_SIZE; i++) {
		dst[i] = a[i] ^ b[i];
	}
}

static void S(uint8_t *vect)
{
	uint8_t i;
...
static void X(uint8_t *dst, const uint8_t *a, const uint8_t *b)
{
	xor512(dst, a, b);
}

static void E(uint8_t *dst, uint8_t *k, uint8_t *m)
{
	uint8_t i;

	uint8_t K[64];

	memcpy(K, k, STRIBOG_BLOCK_SIZE);
...
static void g_N(uint8_t *h, uint8_t *N, uint8_t *m)
{
	uint8_t hash[STRIBOG_BLOCK_SIZE];
	memcpy(hash, h, STRIBOG_BLOCK_SIZE);
...
static void g_0(uint8_t *h, uint8_t *m)
{
	uint8_t hash[64];
	memcpy(hash, h, STRIBOG_BLOCK_SIZE);
...
{
	uint8_t m[STRIBOG_BLOCK_SIZE];

	uint8_t padding;

	while (len >= STRIBOG_BLOCK_SIZE) {
		memcpy(m, message + len - STRIBOG_BLOCK_SIZE, STRIBOG_BLOCK_SIZE);

		g_N(ctx->h, ctx->N, m);

		len -= STRIBOG_BLOCK_SIZE;

		addmod512_uint32_t(ctx->N, ctx->N, STRIBOG_BLOCK_BIT_SIZE);
		addmod512(ctx->S, ctx->S, m);
	}

	padding = STRIBOG_BLOCK_SIZE - len;
	...
	addmod512_uint32_t(ctx->N, ctx->N, len*8);
	addmod512(ctx->S, ctx->S, m);
...
{
	memset(ctx->N, 0x00, STRIBOG_BLOCK_SIZE);
	memset(ctx->S, 0x00, STRIBOG_BLOCK_SIZE);

	ctx->size = !!size;

	if (ctx->size)
		memset(ctx->h, 0x00, STRIBOG_BLOCK_SIZE);
	else
