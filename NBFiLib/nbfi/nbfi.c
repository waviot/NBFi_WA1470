...
void NBFI_Init(nbfi_HAL_st *ptr, ischeduler_st *scheduler, nbfi_dev_info_t *info)
{
    ...
    if ((nbfi_hal == 0) || (nbfi_scheduler == 0))
        while (1)
            ; //HAL and scheduler pointers must be provided
...
void NBFI_Main_Level_Loop()
{
    ...
    if (nbfi_hal->__nbfi_rx_handler)
    {
        ...
        while (1)
        {
            ...
            if (length)
                nbfi_hal->__nbfi_rx_handler(payload, length);
            else
                break;
        }
    }
    ...
    if (nbfi_hal->__nbfi_send_status_handler)
    {
        ...
        while (1)
        {
            ...
            if (ul == 0)
                ul = NBFi_Get_Next_Unreported_UL(LOST);
            nbfi_hal->__nbfi_lock_unlock_loop_irq(NBFI_UNLOCK);
            if (ul)
                nbfi_hal->__nbfi_send_status_handler(*ul);
            else
                break;
        }
    }
...
_Bool NBFi_is_Idle()
{
    ...
    if (NBFi_Packets_To_Send())
        idle = 0;
    else if (last_ack_send_ts && ((nbfi_scheduler->__scheduler_curr_time() - last_ack_send_ts) < WAITALITTLEBIT))
        idle = 0;
    else if (nbfi_active_pkt->state == PACKET_WAIT_FOR_EXTRA_PACKETS)
        idle = 0;
    nbfi_hal->__nbfi_lock_unlock_loop_irq(NBFI_UNLOCK);
    return idle;
}
...
void NBFi_set_Settings(nbfi_settings_t *settings, _Bool persistent)
{
    ...
    if (persistent)
    {
        if (nbfi.rx_phy_channel != settings->rx_phy_channel)
            need_to_send_sync = 1;
        if (nbfi.ul_freq_base != settings->ul_freq_base)
            need_to_send_ul_freq_base = 1;
        if (nbfi.dl_freq_base != settings->dl_freq_base)
            need_to_send_dl_freq_base = 1;
        ...
        if (need_to_send_sync)
        {
            NBFi_Config_Send_Sync(0);
            NBFi_Force_process();
        }
        if (need_to_send_ul_freq_base)
            NBFi_Config_Send_Mode(HANDSHAKE_NONE, NBFI_PARAM_UL_BASE_FREQ);
        if (need_to_send_dl_freq_base)
...
void NBFi_CPU_Reset()
{
    if (nbfi_hal->__nbfi_reset)
        nbfi_hal->__nbfi_reset();
}
