...
static _Bool NBFI_Config_is_high_SNR_for_UP(uint8_t rx_tx)
{
    if (rx_tx & RX_CONF)
    {
        if (current_rx_rate < (NUM_OF_RX_RATES - 1))
        {
            ...
            if (nbfi_state.aver_rx_snr > RX_SNRLEVEL_FOR_UP + rx_delta)
                return 1;
            else
                return 0;
        }
        ...
        {
            if (nbfi_state.aver_rx_snr > RX_SNRLEVEL_FOR_UP + 10)
                return 1;
            else
                return 0;
        }
    ...
    if (rx_tx & TX_CONF)
    {
        if (current_tx_rate < (NUM_OF_TX_RATES - 1))
        {
            ...
            if (nbfi_state.aver_tx_snr > TX_SNRLEVEL_FOR_UP + tx_delta)
                return 1;
            else
                return 0;
        }
        ...
        {
            if (nbfi_state.aver_tx_snr > TX_SNRLEVEL_FOR_UP + 10)
                return 1;
            else
                return 0;
        }
...
void NBFI_Config_Check_State()
{
    ...
    {
        if (nbfi_state.aver_tx_snr)
            nbfi_state.UL_rating = (nbfi_state.aver_tx_snr + TxSNRDegradationTable[current_tx_rate]);
        else
            nbfi_state.UL_rating = 0;
        if (nbfi_state.UL_rating > 40)
            nbfi_state.UL_rating = 40;
        ...
        if (nbfi_state.aver_rx_snr)
            ...
            nbfi_state.DL_rating = 0;
        if (nbfi_state.DL_rating > 40)
            nbfi_state.DL_rating = 40;
        nbfi_state.DL_rating >>= 2;
    }

    if (nbfi.mode == NRX)
        return;
    if (nbfi.additional_flags & NBFI_FLG_FIXED_BAUD_RATE)
        return;
    ...
    if (you_should_dl_power_step_down && (nbfi_state.aver_rx_snr < RX_SNRLEVEL_FOR_UP))
        you_should_dl_power_step_down = 0;
    if (you_should_dl_power_step_up && (nbfi_state.aver_rx_snr > RX_SNRLEVEL_FOR_DOWN))
        you_should_dl_power_step_up = 0;
    ...
    {
        if (!NBFi_Config_Rate_Change(TX_CONF, UP))
            NBFi_Config_Tx_Power_Change(DOWN);
        //return;
    }
    ...
    {
        if (!NBFi_Config_Rate_Change(RX_CONF, UP))
            you_should_dl_power_step_down = (1 << 7);
        return;
    }
...
static _Bool NBFi_Config_Rate_Change(uint8_t rx_tx, nbfi_rate_direct_t dir)
{
    ...
    if ((rx_tx & RX_CONF) && NBFi_Config_Tx_Idle())
    {
        ...
        {
            if (++current_rx_rate > NUM_OF_RX_RATES - 1)
                current_rx_rate = NUM_OF_RX_RATES - 1;
        }
        /* if(dir == DOWN)
        {
    ...
    if ((rx_tx & TX_CONF))
    {
        ...
        {
            if (++current_tx_rate > NUM_OF_TX_RATES - 1)
                current_tx_rate = NUM_OF_TX_RATES - 1;
    ...
    if (rx < current_rx_rate)
        nbfi_state.aver_rx_snr -= rx_delta;
    if (tx < current_tx_rate)
        nbfi_state.aver_tx_snr -= tx_delta;
...
_Bool NBFi_Config_Tx_Power_Change(nbfi_rate_direct_t dir)
{
    if (nbfi.additional_flags & NBFI_FLG_NO_REDUCE_TX_PWR)
        return 0;
...
_Bool NBFi_Config_Parser(uint8_t *buf)
{
    ...
    case WRITE_PARAM_CMD:
        ...
        case NBFI_PARAM_MODE:
            if (buf[1] != 0xff)
                nbfi.mode = (nbfi_mode_t)buf[1];
            if (buf[2] != 0xff)
                nbfi.mack_mode = (nbfi_mack_mode_t)buf[2];
            if (buf[3] != 0xff)
                NBFi_Config_Set_TX_Chan((nbfi_phy_channel_t)buf[3]);
            if (buf[4] != 0xff)
            {
                NBFi_Config_Set_RX_Chan((nbfi_phy_channel_t)buf[4]);
                rf_state = STATE_CHANGED;
            }
            if (buf[5] != 0xff)
                nbfi.tx_pwr = buf[5];
            if (buf[6] != 0xff)
                nbfi.num_of_retries = buf[6];
            break;
        case NBFI_PARAM_HANDSHAKE:
            if (buf[1] != 0xff)
            {
        ...
        case NBFI_PARAM_ANT:
            if (buf[1] != 0xff)
                nbfi.tx_pwr = buf[1];
            if (buf[2] != 0xff)
                nbfi.tx_antenna = buf[2];
            if (buf[3] != 0xff)
            {
                nbfi.rx_antenna = buf[3];
                rf_state = STATE_CHANGED;
            }
            break;
        case NBFI_PARAM_HEART_BEAT:
...
void NBFi_ReadConfig(nbfi_settings_t *settings)
{
    if (settings == 0)
        settings = &nbfi;
    if (nbfi_hal->__nbfi_read_flash_settings == 0)
        goto read_default;

    nbfi_hal->__nbfi_read_flash_settings(settings);

    if ((settings->tx_phy_channel != 0xff) && (settings->tx_phy_channel != 0))
        return;

read_default:

    if (nbfi_hal->__nbfi_read_default_settings)
        nbfi_hal->__nbfi_read_default_settings(settings);

#ifndef NBFI_USE_MALLOC
    if ((settings == 0) && (nbfi.max_payload_len > NBFI_PACKET_SIZE))
...
void NBFi_Config_Set_TX_Chan(nbfi_phy_channel_t ch)
{
    ...
    for (i = 0; i != NUM_OF_TX_RATES; i++)
        if (TxRateTable[i] == ch)
            break;
    if (i == NUM_OF_TX_RATES)
        return;
    nbfi.tx_phy_channel = ch;
...
void NBFi_Config_Set_RX_Chan(nbfi_phy_channel_t ch)
{
    ...
    for (i = 0; i != NUM_OF_RX_RATES; i++)
        if (RxRateTable[i] == ch)
            break;
    if (i == NUM_OF_RX_RATES)
        return;
    nbfi.rx_phy_channel = ch;
...
_Bool NBFi_Config_is_settings_default()
{
    ...
    if ((nbfi.rx_phy_channel != settings.rx_phy_channel))
        return 0;
    if ((nbfi.tx_phy_channel != settings.tx_phy_channel))
        return 0;
    if ((nbfi.nbfi_freq_plan.fp != settings.nbfi_freq_plan.fp))
        return 0;
    return 1;
}
