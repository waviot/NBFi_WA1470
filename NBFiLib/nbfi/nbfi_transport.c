...
#else
    for (uint8_t i = 0; i < NBFI_TX_PKTBUF_SIZE; i++)
        nbfi_TX_pktBuf[i].state = PACKET_FREE;
    for (uint8_t i = 0; i < NBFI_RX_PKTBUF_SIZE; i++)
        nbfi_RX_pktBuf[i].state = PACKET_FREE;
#endif

    for (uint8_t i = 0; i < NBFI_SENT_STATUSES_BUF_SIZE; i++)
        NBFi_sent_UL_stat_Buf[i].id = 0;
...
nbfi_ul_sent_status_t NBFi_Send5(uint8_t *payload, uint8_t length, uint8_t flags)
{
    ...
    if (length < nbfi.max_payload_len)
    {
        ...
        if (!(flags & NBFI_UL_FLAG_NOACK))
            packet->handshake = nbfi.handshake_mode;
        if (flags & NBFI_UL_FLAG_NO_RETRIES)
            packet->retry_num = 0xf0;
    ...
    while (1)
    {
        ...
        if ((pkt->phy_data.SYS) && (pkt->phy_data.payload[0] & 0x80))
        {
            ...
            if (nbfi.mack_mode < MACK_2)
                pkt->state = PACKET_PROCESSED; //NBFi_RxPacket_Free(pkt);
        }
        else
        {
            ...
            for (uint8_t i = 0; i != groupe; i++)
            {
                ...
                if ((i == 0) && (groupe > 1))
                {
                    len = nbfi.max_payload_len - 2;
                    first = 2;
                }
                else
                    len = (memcpy_len >= nbfi.max_payload_len) ? nbfi.max_payload_len : memcpy_len % nbfi.max_payload_len;
                memcpy(data + i * nbfi.max_payload_len - 2 * (i != 0), (void const *)(&NBFi_Get_RX_Packet_Ptr(last_group_iter)->phy_data.payload[first]), len);
                memcpy_len -= len;
                if (NBFi_Get_RX_Packet_Ptr(last_group_iter)->phy_data.ACK)
                    NBFi_Get_RX_Packet_Ptr(last_group_iter)->state = PACKET_CLEARED;
                else
...
void NBFi_ParseReceivedPacket(nbfi_transport_frame_t *phy_pkt, nbfi_mac_info_packet_t *info)
{
    ...
    for (uint8_t k = 0; k != 8; k++)
        sprintf(nbfi_log_string + strlen(nbfi_log_string), "%02X", phy_pkt->payload[k]);
    ...
    if (phy_pkt->SYS)
    {
        ...
        case SYSTEM_PACKET_ACK_ON_SYS: //ACK on system packet received
            if (((nbfi_active_pkt->state == PACKET_WAIT_ACK)) && (phy_pkt->ITER == nbfi_active_pkt->phy_data.ITER))
            {
                ...
                if (nbfi_station_info.info.RTC_MSB & 0x20)
                    rtc_offset = 0xC0 | nbfi_station_info.info.RTC_MSB;
                else
                    rtc_offset = nbfi_station_info.info.RTC_MSB;
                rtc_offset <<= 8;
                rtc_offset |= phy_pkt->payload[6];
                if (rtc_offset && !NBFi_is_Switched_to_Custom_Settings())
                    NBFi_set_RTC_irq(NBFi_get_RTC() + rtc_offset);
                ...
                else if (!NBFi_is_Switched_to_Custom_Settings())
                {
                    ...
                    if (nbfi_station_info.fp.fp == (NBFI_UL_FREQ_PLAN_NO_CHANGE + NBFI_DL_FREQ_PLAN_NO_CHANGE))
                        nbfi_state.bs_id = (((uint16_t)phy_pkt->payload[3]) << 8) + phy_pkt->payload[4];
                    else
        ...
        case SYSTEM_PACKET_GROUP_START_OLD:
            if (was_not_cleared_after_groupe)
                NBFi_Clear_RX_Buffer(-1, 1000 * 60 * 10 /*NBFI_PhyTo_Delay(nbfi.rx_phy_channel)*32*/);
        ...
        case SYSTEM_PACKET_RESET: //software reset
            if ((phy_pkt->payload[1] == 0xDE) && (phy_pkt->payload[2] == 0xAD))
                NBFi_CPU_Reset();
            break;
...
void NBFi_ProcessTasks(struct scheduler_desc *desc)
{
    ...
    //if((rf_busy == 0)&&!NBFi_RF_is_TX_in_Progress())
    {
        ...
        default:
            ...
            if (pkt)
            {
                ...
                if (pkt->state != PACKET_NEED_TO_SEND_RIGHT_NOW)
                    uplink_received_after_send = 0;
                ...
                {
                    wait_RxEnd = 0;
                    nbfi_scheduler->__scheduler_remove_task(&dl_drx_desc);
                }
                ...
                for (uint8_t k = 0; k != 8; k++)
                    sprintf(nbfi_log_string + strlen(nbfi_log_string), "%02X", pkt->phy_data.payload[k]);
    ...
    if (nbfi.mode <= DRX && !NBFi_GetQueuedTXPkt() && (rf_busy == 0) && !NBFi_RF_is_TX_in_Progress())
    {
        ...
        if (rf_state == STATE_OFF)
            ; //nbfi_scheduler->__scheduler_add_task(desc, 0, RELATIVE, SECONDS(10));
        else
            nbfi_scheduler->__scheduler_add_task(desc, 0, RELATIVE, MILLISECONDS(50));
    }
    else
        nbfi_scheduler->__scheduler_add_task(desc, 0, RELATIVE, MILLISECONDS(50));
}
...
void NBFi_set_RTC_irq(uint32_t time)
{
    ...
    nbfi_rtc = time;
    rtc_synchronised = 1;
    if (nbfi_hal->__nbfi_rtc_synchronized)
...
static void NBFi_SendHeartBeats(struct scheduler_desc *desc)
{
    ...
    if (hb_timer == 0)
        hb_timer = rand() % nbfi.heartbeat_interval;
    ...
    {
        hb_timer = 1;
        if (nbfi.heartbeat_num == 0)
            return;
        if (nbfi.heartbeat_num != 0xff)
            nbfi.heartbeat_num--;
        ...
        if (MinVoltage == 0)
            MinVoltage = nbfi_hal->__nbfi_measure_voltage_or_temperature(1);
...
static uint32_t NBFi_UL_Delivery_Time(nbfi_phy_channel_t chan)
{
    ...
    if (nbfi.wait_ack_timeout)
        return nbfi.wait_ack_timeout / 2;
    else
        return NBFI_PhyTo_Delay(chan);
}
...
static uint32_t NBFi_DL_Delivery_Time(nbfi_phy_channel_t chan)
{
    if (nbfi.wait_ack_timeout)
        ...
        return NBFI_PhyToDL_ListenTime(chan) + rand() % NBFI_PhyToDL_AddRndListenTime(chan);
}
