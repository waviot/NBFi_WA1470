...
void nbfi_HAL_read_flash_settings(nbfi_settings_t* settings)
{
	...
	uint16_t dcrc = CRC16(((uint8_t *)settings), sizeof(nbfi_settings_t), 0xA8);

	if(dcrc != *((uint16_t*)(EEPROM_INT_nbfi_data + sizeof(nbfi_settings_t))))
		settings->tx_phy_channel = DL_PSK_200;
}
...
void nbfi_HAL_write_flash_settings(nbfi_settings_t* settings)
{
	...
	for (uint8_t i = 0; i != sizeof(nbfi_settings_t); i++) {
		if (HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_BYTE, EEPROM_INT_nbfi_data + i, ((uint8_t *)settings)[i]) != HAL_OK) {
			break;
		}
	}
	uint16_t dcrc = CRC16(((uint8_t *)settings), sizeof(nbfi_settings_t), 0xA8);
	HAL_FLASHEx_DATAEEPROM_Program(FLASH_TYPEPROGRAMDATA_HALFWORD, EEPROM_INT_nbfi_data + sizeof(nbfi_settings_t), dcrc);
